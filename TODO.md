...
- [x] Define and queue the staged C2/C3 native-context campaign in
  ...
  - [x] Implement and verify a concurrent runner/importer with aligned
    occupancy timestamps and partial stream retention.
  - [x] Run boot/admission, 261,120+1,024 prefill, 1,024+131,072 decode,
    ...
    comparison as a separate factor. The fail-fast campaign completed all C2
    cells and stopped C3 after its exact-occupancy gate observed only two, not
    three, simultaneous near-native requests. The optional ratio remains
    untested and is not needed for the C2-over-C3 decision.
